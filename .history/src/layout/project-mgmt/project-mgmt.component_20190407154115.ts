...
export class ProjectMgmtComponent implements OnInit {
  ...
  isCollapsed2 = true;
  sub: Subscription;
  projectmgmtInfo: Projectmgmt= new Projectmgmt() ;
  ...
  onSubmit() {
    this.projectmgmtInfo.name = this.projectForm.get("name").value;
 
    this.projectmgmtService.save(this.projectmgmtInfo).subscribe(
      result => {
        
        console.log(this.projectmgmtInfo)
      },
